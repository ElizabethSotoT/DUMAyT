...
session_start();
	...
	if(!empty($_POST))
	{
		...
		$alert='';
		if(empty($_POST['id_licencia']) || empty($_POST['cuota_anual']) || empty($_POST['forma_pago']) || empty($_POST['fecha_pago']) || empty($_POST['fecha_inicio']) || empty($_POST['fecha_final']) || empty($_POST['id_persona']) || empty($_POST['id_anuncio']))
		{
...
<?php include "includes/wrapp.php"; ?>
    ...
    <form action="" method="post">
        ...
        <select name="id_anuncio" id="id_anuncio"> 
        	<option value="<?php echo $id_anuncio; ?>" selected><?php echo $ubicacion; ?></option> 
            <?php
                if ($result_anuncio > 0)
                {
